  ...
  <a-card>
      ...
      <a-tab-pane key="1" tab="企业面试">
        <div class="Searchbox clearfix">
          ...
          <a-form-model :model="searchjson">
            ...
            <a-form-model-item label="应聘职位：" :labelCol="{span: 8}" :wrapperCol="{span: 16}" style="width:200px;">
              <a-input autocomplete="autocomplete" placeholder="请输入" v-model="searchjson.vpndeliverGet.jobName">
              </a-input>
            </a-form-model-item>
    ...
    <a-modal centered title="邀请面试" :visible="visible" :confirm-loading="confirmLoading" @ok="onSubmit" @cancel="Cancel">
      <a-form-model ref="ruleForm" :model="addjson" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-model-item label="面试地点" prop="adress">
          <a-input autocomplete="autocomplete" placeholder="面试地点" v-model="addjson.adress">
          </a-input>
        </a-form-model-item>
...
<script>
  import {GetZJQYdeliver,GetZJWTdeliver,asyncvpnDeliverDetailAdd} from '@/services/job'
  export default {
    components: {},
    data() {
      return {
        ...
        rules: {
          ...
          adress: [{ required: true, message: '请输入面试地址', trigger: 'blur'}]
        },
        visible:false,
    ...
    methods: {
      ...
      findList(){
        if(this.tag=='1'){
          GetZJQYdeliver(this.searchjson).then(result => {
          ...
          GetZJWTdeliver(this.searchjson).then(result => {
            if(result.data.code==1){
              this.DataList=result.data.data.vpndeliverGetResp
            }else{
              this.$message.error(result.data.msg)
