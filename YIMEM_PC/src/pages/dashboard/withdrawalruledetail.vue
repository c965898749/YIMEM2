...
<script>
  import {asyncwithdrawRuleDetail} from '@/services/dashboard'
  export default {
    data() {
      return {
    ...
    methods: {
      getData(){
        asyncwithdrawRuleDetail(this.wiId).then(result => {
          this.loading = false
          if(result.data.code==1){
            this.Detail=result.data.data.vpnwithdrawRuleGetResp
          }else{
