...
<script>
  ...
  import {asyncprotocolAdd,asyncprotocolDetail} from '@/services/dashboard'
  ...
  export default {
    ...
    created() {
      if(this.json.prId != null){
        this.getData()
      }
    },
    methods: {
      getData(){
        asyncprotocolDetail(this.json.prId).then(result => {
          this.loading = false
          if(result.data.code==1){
            this.json=result.data.data.vpnprotocolGetResp
      ...
      onSubmit(){
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            let requests=''
            if(this.json.prId){
              requests={vpnprotocolUpdate:this.json}
            }else{
              requests={vpnprotocolAdd:this.json}
            }
            this.loading=true
            asyncprotocolAdd(this.json.prId?'edit':'add',requests).then(result => {
              if(result.data.code==1){
                this.$message.success(result.data.msg, 3)
                this.$router.push({path:'/dashboard/agreement/list',query: {toUser:this.json.toUser}})
              }else{
                this.$message.error(result.data.msg)
              }
              this.loading=false
