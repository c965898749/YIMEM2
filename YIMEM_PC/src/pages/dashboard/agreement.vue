<template>
  <a-card>
    ...
    <a-tabs :default-active-key="json.vpnprotocolGet.toUser" @change="callback">
      <a-tab-pane key="1" tab="求职者">
        <a-list class="listbox" :loading="loading" item-layout="horizontal" :data-source="dataList">
          ...
          <a-list-item slot="renderItem" slot-scope="item">
            <a slot="actions" @click="Del(item.prId)" v-if="button.del"><a-icon type="delete" /> 删除</a>
            <template v-if="button.edit">
            <a slot="actions" @click="Edit(item.prId)"><a-icon type="form" /> 编辑</a>
            <a slot="actions" @click="SetState(item.prId,0)" v-if="item.state==1"><a-icon type="check-circle" /> 停止</a>
            <a slot="actions" @click="SetState(item.prId,1)" v-else><a-icon type="stop" /> 使用</a>
            </template>
            <a-list-item-meta :description="item.content">
              <a slot="title" @click="Detail(item.prId)">{{ item.title }}</a>
            </a-list-item-meta>
      ...
      <a-tab-pane key="2" tab="分销者" force-render>
        ...
        <a-list class="listbox" :loading="loading" item-layout="horizontal" :data-source="dataList">
          ...
          <a-list-item slot="renderItem" slot-scope="item">
            <a slot="actions" @click="Del(item.prId)"><a-icon type="delete" /> 删除</a>
            <a slot="actions" @click="Edit(item.prId)"><a-icon type="form" /> 编辑</a>
            <a slot="actions" @click="SetState(item.prId,0)" v-if="item.state==1"><a-icon type="check-circle" /> 停止</a>
            <a slot="actions" @click="SetState(item.prId,1)" v-else><a-icon type="stop" /> 使用</a>
            <a-list-item-meta :description="item.content">
...
<script>
  ...
  import {asyncprotocolGet,asyncprotocolAdd,asyncprotocolDel} from '@/services/dashboard'
  export default {
    data() {
      return {
        json:{
    ...
    methods: {
      callback(key) {
        this.json.vpnprotocolGet.toUser=key
        this.Search()
      },
      findList() {
        this.getData(res => {
      ...
      getData(callback){
        asyncprotocolGet(this.json).then(result => {
          this.loading = false
      ...
      Detail(prId){
        this.$router.push({path:'/dashboard/agreement/detail',query: {prId:prId,toUser:this.json.vpnprotocolGet.toUser}})
      },
      Add(){
        this.$router.push({path:'/dashboard/agreement/add',query: {toUser:this.json.vpnprotocolGet.toUser}})
      },
      Edit(prId){
        this.$router.push({path:'/dashboard/agreement/edit',query: {prId:prId,toUser:this.json.vpnprotocolGet.toUser}})
      },
      SetState(prId,state){
        const _self=this
        this.$confirm({
          ...
          onOk() {
            ...
            asyncprotocolAdd('edit',requests).then(result => {
              if(result.data.code==1){
      ...
      Del(prId){
        ...
        this.$confirm({
          ...
          //content: '',
          onOk() {
            asyncprotocolDel(prId).then(result => {
              if(result.data.code==1){
                _self.$message.success(result.data.msg, 3)
                _self.findList()
              }else{
