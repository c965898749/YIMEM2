<template>
	<view class="content">
		<scroll-view scroll-y="true" class="scrollBox" @scrolltolower="reachBottom">
			<!-- 分销者和求职者 -->
			<view class="" v-if="userState==1 || userState==2 ">
				<view class="scrollList">
					<view class="scrollItem" @click="goToJobDetail(item)" v-for="(item,index) in jobList" :key="index">
						<view class="itemTop">
							<view class="oneTop">
								<view class="left-row">
								...
								<view v-if="userState==2" >
									...
									<text class="redText">{{item.job.brokerage}}元/人</text>
								</view>
							</view>
						</view>
						<view class="company-info">
							<view class="oneMid">{{item.job.cityName}}{{item.job.countryName}} | {{item.job.jobExpName || '暂无'}} | {{item.job.educationExpName || '暂无'}}
							</view>
							<view class="textList">
								<text class="lightspot" v-for="(one,indexp) in item.job.jobCustomLightspot"
									:key="indexp">
									{{one.name}}
								</text>
								<text class="lightspot" v-for="(two,indext) in item.job.jobLightspot" :key="indext">
									{{two.name}}
								</text>
							</view>
						</view>
						<view class="botComp">
							<view class="oneBot" @click.stop="cancelCollect(item)">
								<text class="oneMid">取消收藏</text>
							</view>
							<view class="shuTxet"></view>
							<view class="oneBot" @click.stop="commitJob(item)">
								<text class="oneMid" style="color: #2E73FF;">我要应聘</text>
							</view>
						</view>
					</view>
			...
			<!-- 企业端 -->
					...
					<!-- <view class="" v-if="userState==1 || userState==2 ">
						...
						<view class="allList" v-if="userState==3">
							...
							<view class="scrollList">
								<view class="oneItem" v-for="(item,index) in compJobList" :key="index" @click="goToPage(item)" >
									<view class="leftItem">
									...
									<view class="rightItem">
										...
										<view class="pContent">
											<view class="">
												<view class="pText">
													<text class="pTip">{{item.userSex==0?'女':'男'}} | {{item.user.age}}岁 | {{item.education || '未知'}}｜{{item.user.workage || 0 }}年｜{{item.cityName || '未知'}}</text>
												</view>
												<view class="pText">
													<text>应聘职位：{{item.job.name || '暂无'}}</text>
												</view>
												<view class="">
													<text class="workMoney"
														v-if="item.job.isAllDay==1">{{Math.round(item.job.lowSalary/100)/10}}-{{Math.round(item.job.higSalary/100)/10}}k</text>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				vpncollectGetsvc(data).then(res => {
					if (res.code == 1) {
						if (that.pageIndex == 1) {
							that.jobList = res.data.vpncollectGetResp;
						} else {
							that.jobList = [...that.jobList, ...res.data.vpncollectGetResp]
						}
						if (res.data.vpncollectGetResp.length < that.pageSize) {
							that.canLoad = false;
						} else {
							that.pageIndex++
						}
			...
			getCompJobList() {
				...
				vpncompanycollectdevGetsvc(data).then(res => {
					if (res.code == 1) {
						if (that.pageIndex == 1) {
